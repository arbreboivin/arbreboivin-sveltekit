<script lang="ts">
	...
	const currentIndex = $derived(servicesEn.findIndex((s) => s.slug === service.slug));
	const prevService = $derived(currentIndex > 0 ? servicesEn[currentIndex - 1] : null);
	const nextService = $derived(currentIndex < servicesEn.length - 1 ? servicesEn[currentIndex + 1] : null);
</script>
...
<svelte:head>
	...
	<meta property="og:url" content="{site.url}/en/services/{service.slug}" />
	<link rel="canonical" href="{site.url}/en/services/{service.slug}" />
</svelte:head>
...
<div class="relative h-[55vh] min-h-[320px] w-full overflow-hidden">
	...
	<div class="absolute inset-x-0 bottom-0 px-4 pb-8 sm:px-8">
		<div class="container-site">
			<nav class="mb-3 flex items-center gap-2 text-xs text-green-300" aria-label="Breadcrumb">
				<a href="/en" class="hover:text-white transition-colors">Home</a>
				<span aria-hidden="true">›</span>
				<a href="/en/services" class="hover:text-white transition-colors">Services</a>
				<span aria-hidden="true">›</span>
	...
	<div class="container-site">
		...
		<div class="grid grid-cols-1 gap-12 lg:grid-cols-3">
			...
			<aside class="rounded-xl bg-[#f4f9f4] p-6 border border-[#d4e8d4] h-fit">
				...
				<a
					href="/en/contact"
					class="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-[#2d6a2d] px-5 py-3 text-sm font-bold uppercase tracking-wide text-white shadow transition-colors hover:bg-[#1a4a1a]"
				>
					Request a quote
				...
				<a
					href={site.phoneHref}
					class="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border-2 border-[#2d6a2d] px-5 py-3 text-sm font-semibold text-[#2d6a2d] transition-colors hover:bg-[#2d6a2d] hover:text-white"
				>
					<svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
...
<nav class="bg-white border-t border-gray-100 py-8" aria-label="Previous / next service">
	<div class="container-site flex items-center justify-between gap-4">
		{#if prevService}
			<a href="/en/services/{prevService.slug}" class="group flex items-center gap-3 rounded-lg border border-gray-200 px-5 py-3 text-sm font-semibold text-gray-700 transition-colors hover:border-[#2d6a2d] hover:text-[#2d6a2d]">
		...
		<a href="/en/services" class="text-xs font-semibold uppercase tracking-widest text-gray-400 transition-colors hover:text-[#2d6a2d]">
			All services
		</a>

		{#if nextService}
			<a href="/en/services/{nextService.slug}" class="group flex items-center gap-3 rounded-lg border border-gray-200 px-5 py-3 text-sm font-semibold text-gray-700 transition-colors hover:border-[#2d6a2d] hover:text-[#2d6a2d]">
...
<section class="bg-[#1a4a1a] px-4 py-14 text-center text-white">
	<p class="mb-2 text-xs font-bold uppercase tracking-widest text-green-400">Free estimate</p>
	...
	<p class="mx-auto mb-8 max-w-xl text-sm text-green-200">
		Contact us today. An expert arborist will get back to you quickly for your {service.title.toLowerCase()}.
	</p>
	<div class="flex flex-col items-center justify-center gap-3 sm:flex-row">
		<a href="/en/contact" class="inline-flex items-center gap-2 rounded-lg bg-white px-8 py-3.5 text-sm font-bold uppercase tracking-wide text-[#1a4a1a] shadow transition-colors hover:bg-green-50">
			Free quote
			<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" />
			</svg>
		</a>
		<a href={site.phoneHref} class="inline-flex items-center gap-2 rounded-lg border-2 border-white/60 px-8 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-white/10">
			<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
			</svg>
			{site.phone}
