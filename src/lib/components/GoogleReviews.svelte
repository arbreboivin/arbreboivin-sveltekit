...
<section class="relative py-16 md:py-20 overflow-hidden bg-[#f5f0e8]">
	...
	<div class="container-site relative">
		...
		<div class="text-center mb-10">
			<h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Avis Google</h2>
			<p class="text-gray-500 text-base">Des clients satisfaits à Montréal et sur la Rive-Nord.</p>
		</div>

		<!-- Note globale -->
		<div class="text-center mb-8">
			<p class="font-bold text-gray-900 text-lg">{googleReviews.businessName}</p>
			<div class="flex items-center justify-center gap-2 mt-1">
				...
				<span class="font-bold text-gray-800">{googleReviews.rating.toFixed(1)} / 5</span>
				<span class="text-gray-500 text-sm">({googleReviews.totalReviews} avis)</span>
			</div>
		</div>
				...
				{#each visibleReviews as review, i}
					...
					<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3">
						...
						<div class="flex items-center gap-3">
							...
							<div>
								<p class="font-semibold text-gray-900 text-sm leading-tight">{review.author}</p>
						...
						<!-- Logo Google -->
						<div class="mt-auto pt-2 flex items-center gap-1.5 border-t border-gray-50">
							...
							<span class="text-xs text-gray-400">Avis Google</span>
						</div>
					</div>
				{/each}
			</div>
