...
<header
	class="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-100 shadow-sm transition-colors duration-200 dark:bg-[#0c1222]/98 dark:border-slate-700/50"
>
	<div class="container-site">
			...
			<div class="flex items-center gap-2 sm:gap-3">
				<a href="/" class="flex items-center gap-3 group shrink-0" onclick={closeMobile}>
					...
					<div class="flex flex-col leading-tight">
						...
						<span class="text-xs text-gray-500 dark:text-gray-400">{site.domain}</span>
					</div>
				</a>
			</div>
		...
		<!-- CTA desktop : téléphone → Jour/Nuit → FR/EN → bouton devis → mobile -->
		<div class="flex items-center gap-2 sm:gap-3 md:gap-4 ml-2 sm:ml-4 md:ml-8 flex-shrink-0">
			...
			<!-- Bouton Jour/Nuit (comme CEDRÈDE : à gauche du sélecteur de langue) -->
			<span class="flex items-center gap-1.5 max-md:min-h-[44px] max-md:min-w-[44px] max-md:justify-center" title={isDark ? 'Mode clair' : 'Mode nuit'}>
				<button
					type="button"
					onclick={() => themeStore.toggle()}
					class="inline-flex h-10 w-10 min-w-[2.5rem] flex-shrink-0 items-center justify-center rounded-full border-2 border-amber-400/80 bg-[#e4a853] text-white transition hover:bg-[#d4983a] hover:border-amber-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 max-md:h-11 max-md:w-11 max-md:min-h-[44px] max-md:min-w-[44px] dark:border-amber-500/70"
			...
			<!-- FR | EN -->
			<div class="hidden md:flex ml-2 items-center rounded-lg border border-gray-200 overflow-hidden dark:border-slate-600" role="group" aria-label="Langue">
				{#if hrefFr !== null}
					<a
						href={hrefFr}
			...
			<a href={isEnglish ? '/en/contact' : site.cta.href} class="btn-primary hidden md:inline-flex text-base py-3 px-6 rounded-xl">
				...
				<button
					...
					class="md:hidden p-2 rounded-md text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-[#2d6a2d] dark:text-gray-300 dark:hover:bg-slate-800 min-h-[44px] min-w-[44px] flex items-center justify-center"
					aria-label="Menu"
	...
	<!-- Drawer mobile : simplifié et plus lisible sur téléphone -->
	{#if mobileOpen}
		<div class="md:hidden border-t border-gray-100 bg-white shadow-lg dark:bg-[#111827] dark:border-slate-700">
			<nav class="container-site py-5 flex flex-col gap-0" aria-label="Navigation mobile">
				{#each (isEnglish ? navEn : site.nav) as item}
					<a
						href={item.href}
						onclick={closeMobile}
						class="min-h-[48px] flex items-center px-4 py-3.5 text-base font-medium rounded-lg transition-colors active:bg-gray-100 dark:active:bg-slate-800
							{page.url.pathname === item.href
							? 'text-[#2d6a2d] bg-green-50 dark:text-green-400 dark:bg-slate-800'
							: 'text-gray-700 hover:text-[#2d6a2d] hover:bg-gray-50 dark:text-gray-300 dark:hover:text-green-400 dark:hover:bg-slate-800'}"
					>
				...
				{/each}
				<div class="pt-4 mt-3 border-t border-gray-100 dark:border-slate-700 flex flex-col gap-3">
					<button
						type="button"
						onclick={() => { themeStore.toggle(); closeMobile(); }}
						class="flex items-center justify-center gap-2 rounded-xl border-2 border-amber-500 bg-amber-400/20 px-5 py-4 text-base font-bold text-amber-800 transition active:opacity-90 dark:bg-amber-500/30 dark:text-amber-200 dark:border-amber-400 min-h-[48px]"
						aria-label={isDark ? 'Passer en mode clair' : 'Passer en mode nuit'}
					>
						{#if isDark}
